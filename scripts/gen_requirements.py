...
# server deps (dict of name -> version constraint)
server_deps = pyproject["tool"]["poetry"]["group"]["server"]["dependencies"]

# write train requirements
Path("requirements").mkdir(exist_ok=True)
with open("requirements/train.txt", "w") as f:
    for dep in deps:
        f.write(dep + "\n")
